package tech.tongyu.bct.quant.library.numerics.ad;
...
public class Backwarder {
    ...
    public Backwarder(String[] names, double[] values, int order) {
        if (order != 1 && order != 2)
            throw new CustomException(ErrorCode.COMPUTATION_ERROR,
                    String.format("Quantlib: AD仅支持一阶及二阶导数计算。请求：%d", order));
        this.order = order;
    ...
    public String sum(String x, String y) {
        ...
        applyTwo(name, get(x), get(y),
                (u, v) -> u + v,
                (u, v) -> new double[]{1.0, 1.0},
                (u, v) -> new double[][]{{0.0, 0.0}, {0.0, 0.0}});
        return name;
    }
    ...
    public String mul(String x, String y) {
        ...
        applyTwo(name, get(x), get(y),
                (u, v) -> u * v,
                (u, v) -> new double[]{v, u},
                (u, v) -> new double[][]{{0.0, 1.0}, {1.0, 0.0}
                });
        return name;
    }
    ...
    public String div(String x, String y) {
        ...
        applyTwo(name, get(x), get(y),
                (u, v) -> u / v,
                (u, v) -> new double[]{1.0 / v, -u / (v * v)},
                (u, v) -> new double[][]{{0.0, -1.0 / (v * v)}, {-1.0 / (v * v), 2.0 * u / (v * v * v)}});
        return name;
    }
