...
import tech.tongyu.bct.service.quantlib.common.utils.DateUtils;
...
public class VolCalendar {
    @JsonProperty("class")
    private final String type = VolCalendar.class.getSimpleName();
    ...
    public double getEffectiveNumDays(LocalDateTime start, LocalDateTime end) {
        ...
        if (start.toLocalDate().isEqual(end.toLocalDate())) {
            effectiveNumDays = start.until(end, ChronoUnit.NANOS) / Constants.NANOSINDAY;
        ...
        // start
        double extra = 1.0 - start.toLocalTime().toNanoOfDay() / Constants.NANOSINDAY;
        double w = 0.0;
        if (start.toLocalDate().getDayOfWeek().getValue() < 6 && !specialDates.contains(start.toLocalDate()))
            w = 1.0;
        else {
        ...
        // end
        extra = end.toLocalTime().toNanoOfDay()/Constants.NANOSINDAY;
        w = 0.0;
...
//            throw new RuntimeException("Required time is after end time.");
        ...
        if (effectiveT <= 0) // no effective days between start and end
            return varStart;
    ...
    public double getWeight(LocalDateTime t) {
        if (!DateUtils.isWeekend(t) && !specialDates.contains(t.toLocalDate()))
            return 1.0;
        double weight = 0.0;
        if (DateUtils.isWeekend(t))
