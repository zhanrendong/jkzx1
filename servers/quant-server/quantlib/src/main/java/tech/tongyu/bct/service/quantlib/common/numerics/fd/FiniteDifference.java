...
/**
 * calculate the value, first and second derivatives for a given function and its arguments.
 */
...
public class FiniteDifference {
    ...
    /*
     * create a FiniteDifference instance to calculate the first and second derivatives of the assigned function.
     */
    ...
    /*
     * get the value of the function with specified variables
     */
    ...
    private double[] defaultStepSize(double[] x) {
        ...
        for (int i = 0; i < len; ++i) {
            dx[i] = abs(x[i]) * 0.01;
            if (dx[i] < Constants.SMALL_NUMBER)
                dx[i] = 0.0001;
        }
        return dx;
    }
