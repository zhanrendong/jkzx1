...
def intraday_risk_by_underlyer_report(positions, underlyer_positions, domain, headers, pricing_environment):
    ...
    quotes = get_underlyer_quotes(underlyers, datetime.now(), domain, headers)[['last']]
    quotes.reset_index(inplace=True)
    quotes.rename(columns={'instrumentId': 'underlyerInstrumentId', 'last': 'underlyerPrice'}, inplace=True)
    ...
    risk_report = risk_report.merge(multiplier, on='underlyerInstrumentId', how='left')
    risk_report['deltaCash'] = np.float64(risk_report['netDelta']) * np.float64(risk_report['underlyerPrice']) \
                               * np.float64(risk_report['underlyerMultiplier'])
    risk_report['createdAt'] = str(datetime.now())
