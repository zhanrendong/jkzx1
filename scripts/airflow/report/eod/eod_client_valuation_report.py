...
def option_total_pnl(positions, risks, cash_flows):
    ...
    for r in risks.values():
        if (not r['isSuccess']) or (r['price'] is None):
            continue
        if r['positionId'] not in position_pnl:
            continue
        pnl = position_pnl[r['positionId']]
        pnl['marketValue'] = r['price']
        pnl['pnl'] = r['price']
    # cash flows
    for c in cash_flows.values():
        if c['positionId'] not in position_pnl:
            continue
        pnl = position_pnl[c['positionId']]
        pnl['open'] = pnl['open'] + c['open']
        pnl['unwind'] = pnl['unwind'] + c['unwind']
        pnl['settle'] = pnl['settle'] + c['settle']
        pnl['pnl'] = pnl['pnl'] + c['open'] + c['unwind'] + c['settle']

    return list(position_pnl.values())
...
def client_valuation(positions, pnls, valuation_date, domain, headers):
    ...
    instrument_ids = set()
    for p in positions.values():
        clients.add(p['counterPartyCode'])
        position2client[p['positionId']] = p['counterPartyCode']
        instrument_ids.add(p['underlyerInstrumentId'])
    # empty report
    valuations = {}
    for c in clients:
        valuations[c] = {
    ...
    # position info
    quotes = get_underlyer_quotes(list(instrument_ids), datetime.now(), domain, headers)
    for p in positions.values():
        p_id = p['positionId']
        client = position2client.get(p_id, None)
        if client is None:
            continue
        valuations[client]['positions'][p_id] = {
            ...
            'underlyerInstrumentId': p['underlyerInstrumentId'],
            'quantity': quantity(p['notionalAmount'], p['notionalAmountType'], p['annualized'],
                                 p.get('term', None), p.get('daysInYear', 365), p['initialSpot'],
                                 p['underlyerMultiplier']),
            'tradeDate': p['tradeDate'],
            'expirationDate': p.get('expirationDate', None),
            'premium': premium_amount(p['direction'], p['premium'], p['premiumType'], p['notionalAmount'],
                                      p['notionalAmountType'], p['annualized'], p.get('term', None),
                                      p.get('daysInYear', 365), p['initialSpot'], p['underlyerMultiplier']),
            'lcmEventType': p['lcmEventType'],
            'tradeStatus': 'LIVE',
            'optionType': p.get('optionType', None),
            'strike': p.get('strike', None),
            'initialSpot': p['initialSpot'],
            'underlyerPrice': quotes.get(p['underlyerInstrumentId'], {'close', None})['close'],
            'settle': None,
            'settlePnl': None,
            'marketValue': None,
            'marketValuePnl': None
