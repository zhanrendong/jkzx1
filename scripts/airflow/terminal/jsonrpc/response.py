...
from terminal.jsonrpc import status
...
class DictResponse(Response):
    ...
    def __str__(self) -> str:
        ...
        return json.dumps(sort_dict_response(self.deserialized()), cls=self.json_encoder)
...
class BatchResponse(Response):
    ...
    def __str__(self) -> str:
        ...
        # spec
        return json.dumps(dicts, cls=self.json_encoder) if len(dicts) else ""


def create_exception_response(exc, debug, code=3200):
    return ExceptionResponse(exc=exc, debug=debug, id=1, code=code)
